...
-- Everything runs fine. Most of these tests are out of the assignment pdf, a couple
-- more are my own that demonstrate another edge case of some sort.
--
-- There are two tests cases I can't seem to fix by fiddling with type signatures.
-- The good news is if you try those cases (i.e. make that function call) the correct
-- result is displayed; those functions work properly for the given case, they just
-- don't execute from the tests.
main =  runTestTT (TestList [
    ...
    (TestCase (assertEqual ""   []          (mynthtail 5 [1,2,3,4]))),
	...
	(TestCase (assertEqual ""	[]	        (mylast []::[Char]))),
...
myremoveduplicates_pmh :: Eq a => [a] -> [a] -> [a]
